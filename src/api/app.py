...
# API Key + Admin Key
# -------------------------
PUBLIC_PATHS = {"/health", "/__routes", "/openapi.json", "/docs", "/redoc"}
...
async def api_key_middleware(request: Request, call_next):
    ...
    # quota / limits here if you have them...

    try:
        resp = await call_next(request)
    except Exception as e:
        # Return JSON 500 that the browser can read
        resp = JSONResponse({"detail": "Internal Server Error", "error": str(e)}, status_code=500)

    return _add_cors_headers(resp, request)

# -------------------------
# PATCHED compute + routes
# -------------------------
def _compute_signal(asset: str, mode: str) -> dict:
    try:
        df = load_price_data(asset)
    except FileNotFoundError as e:
        # This is your current crash. Turn it into a clean 404.
        raise HTTPException(
            status_code=404,
            detail=f"Price data not found for asset={asset}. ({str(e)})",
        )
    except Exception as e:
        # Keep other issues explicit
        raise HTTPException(status_code=500, detail=f"Failed to load price data: {str(e)}")

    feats = build_price_feature_set(df)

    if mode == "combined":
        sig = generate_combined_signal(feats)
    else:
        sig = generate_rule_based_signal(feats)
...
def signal_explain(asset: str = "BTC-USD", mode: str = "combined"):
    payload = _compute_signal(asset=asset, mode=mode)
    # If your existing code builds an explanation object, keep it.
    # Minimal placeholder:
    return {"explain": {"summary": "OK", "asset": asset, "mode": mode, "signal": payload}}

@app.get("/backtest")
def backtest(asset: str = "BTC-USD", mode: str = "combined"):
    payload = _compute_signal(asset=asset, mode=mode)
    # Minimal placeholder. Replace with your real backtest logic if present.
    return {"total_return": 0.0, "buy_hold_total_return": 0.0, "trades": 0, "signal": payload}
